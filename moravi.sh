...
#						Sept. 2014
#					   Andrew J Moodie
#				https://github.com/amoodie
#
# This script is to be placed in the same directory as that which contains the raw
# unextracted and unexplored downloaded data -- i.e. exactly as it was downloaded.
# For example, if you want the complete pasted grid file in folder `your_dir`, then
...
#	Bulk Order *****/
#
# Where `Bulk Order *****/` is the folder downloaded from Earth Explorer
# and Bulk Download Application.
...
#
	for file in ./data/*
		...
		#
		filename_out=${lat_dir}${lat_num}${lon_dir}${lon_num}
		xyz2grd $file -G./grd_files/${filename_out}.grd \
...
#
	...
	# iterate the .out files that have size > 0
	# make the missing files
	# named from filename and info in file
	# put in correct folder
	# delete temp and out files
	for file in *.out
		do
